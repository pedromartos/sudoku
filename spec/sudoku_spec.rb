...
describe Sudoku do
  context 'Validate Board' do
    context 'when board is correct' do
      it 'returns true' do
        items = [
          [5, 3, 4, 6, 7, 8, 9, 1, 2],
          [6, 7, 2, 1, 9, 5, 3, 4, 8],
        ...
        expect(sudoku.valid?).to be_truthy
      end
    end

    context 'when board is incorrect' do
      it 'returns false' do
        items = [
          [6, 7, 2, 1, 9, 5, 3, 4, 8],
          [1, 9, 8, 3, 4, 2, 5, 6, 7],
        ...
        expect(sudoku.valid?).to be_falsey
      end
    end
  end

  context 'Validate squares' do
    context 'when number of values is equal to 9' do
      context 'no number repeats on the square' do
        it 'returns true' do
          items = [
            [5, 3, 4, 6, 7, 8, 9, 1, 2],
            [6, 7, 2, 1, 9, 5, 3, 4, 8],
          ...
          expect(sudoku.valid_squares?).to be_truthy
        end
      end
      ...
      context 'one or more number are repeated' do
        it 'returns true' do
          ...
          expect(sudoku.valid_squares?).to be_falsey
        end
      end
    end
  end
